...
router.post('/register', (req, res) => {
  ...
  if (!validator.isRealPassword(req.body.password)) {
    errors.push({
      passwordError:
        'Please ensure that your password has Uppercase, Lowercase, and digit in range [5-10]',
    });
  }
          ...
          insertUser(newUser)
            .then((inserted) => {
              if (inserted) {
                req.flash(
                  'success_msg',
                  'Welcome to XrayTalk, start chatting with friends!'
                );
                res.redirect('/');
              }
            })
