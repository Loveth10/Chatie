...
var XrayTalkApp = (function (Media_UI, Utils) {
  ...
  function updateFriendsList(friends) {
    Media_UI.friendsListObj = Media_UI.friendsListObj.empty();
    if (friends) {
      //-- this user has friend(s)
      for (var i = 0; i < friends.length; i++) {
        //-- loop through the data
        Media_UI.friendsListObj.append(
          Utils.generateFriendListTemplate(friends[i], i)
        );
      }
    } else {
      //-- this user has no friend yet
  ...
  function updateSearchResultList(users) {
    ...
    if (users) {
      ...
      for (var i = 0; i < users.length; i++) {
        Media_UI.searchResultObj.append(
          Utils.generateSearchResultTemplate(users[i], i)
        );
      }
    } else {
      //-- no user with the supplied user name
