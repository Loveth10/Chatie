...
var Utils = (function () {
  ...
  function generateMessageTemplate(message, imgSrc, userType) {
    ...
    if ((message, imgSrc, userType)) {
      ...
      var messageTemplate =
        '<img src="%%src%%" class="chat-thumbnail"/>' +
        '<div class="message">' +
        '<span>%%message%%</span>' +
        '</div>';
  ...
  function generateSearchResultTemplate(searchResult, btnIndex) {
    ...
    if (searchResult) {
      html =
        '<a href="#%%userName%%" class="newFriend" id="newFriendBtn' +
        btnIndex +
        '">' +
  ...
  function generateFriendListTemplate(friendData, index) {
    ...
    if (friendData) {
      html =
        '<a href="#%%ROOMID%%" class="friend-link" id="btnFriend-' +
        index +
        '">' +
        '<img src="%%FRIENDIMG%%" class="chat-thumbnail" />' +
        '<p class="user-name-text">%%userName%% </p>' +
        '</a>';
  ...
  function generateSearchBar() {
    ...
    var html =
      '<input id="searchData" class="form-control" type="text" placeholder="Find new Friends by their Username"/>' +
  ...
  function generateChatTemplate() {
    ...
    var html =
      ...
      '<div class="message-sender-panel">' +
